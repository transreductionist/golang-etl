...
func (s *SQLWriter) ProcessData(d data.JSON, outputChan chan data.JSON, killChan chan error) {
    ...
    dd, _ := getSQLWriterData(d, s)

    err := data.ParseJSONSilent(dd, &wd)
...
func getSQLWriterData(d data.JSON, s *SQLWriter) (data.JSON, error) {
    obj, _ := data.ObjectsFromJSON(d)
    if writeData, ok := obj[0][s.TableName]; ok {
        return data.NewJSON(writeData)
    }
    return d, nil
}
