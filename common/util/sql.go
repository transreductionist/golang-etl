...
// ConnectionString will give you database connection string
func ConnectionStringDB(con Database) string {

    connectionFormat := "%s:%s@(%s:%d)/%s"
    if con.ParseTime == true {
        connectionFormat = "%s:%s@(%s:%d)/%s?parseTime=%t"
    }

    connString := fmt.Sprintf(connectionFormat,
        con.UserID,
        con.PassWord,
        con.Host,
        con.Port,
...
func getConnectionFormat(parseTime bool) string {
    connectionFormat := "%s:%s@(%s:%d)/%s"
    if parseTime == true {
        connectionFormat = "%s:%s@(%s:%d)/%s?parseTime=%s"
    }
    return connectionFormat
}

// DBconnect will give you an *sqlx.DB object to work with database
func DBConnect(connection string) *sql.DB {
...
// GetDataFromSQLQuery is a util function that, given a properly initialized sql.DB
// and a valid SQL query, will handle executing the query and getting back data.JSON
// objects. This function is asynchronous, and data.JSON should be received on the return
